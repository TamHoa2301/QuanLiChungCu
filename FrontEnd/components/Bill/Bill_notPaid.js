...
const Bill_notPaid = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        ...
        const response = await axios.get(`https://tuantunguyenq23.pythonanywhere.com/users/${userId.id}/get-bills-not-paid/`);
        setBills(response.data);
        console.log(response.data)
        console.log(setBills.data)
        setError('');
      } catch (error) {
